...
static struct spi_device* spidev[8] = {};

int spic_rw(unsigned txBytes, const char* txBuf, unsigned rxBytes, char* rxBuf)
{
    int ret = FAIL;
...
s32 spi_nor_write(u32 page_num, u32 page_cnt, const void* buf)
{
    u32 page_addr = page_num * SYSTEM_PAGE_SIZE;
...
static int f_read(struct file* filep, char* buff, size_t nSec, loff_t* offp)
{
    ...
    ret = spi_nor_read(0, nSec, rxData);
    if(!ret)
    {
...
static int create_dev(int index)
{
    ...
    alloc_chrdev_region(&devid, 0, 1, "nor");
    myCdev = cdev_alloc();
    ...
    devClass = class_create(THIS_MODULE, "nor");
    if(IS_ERR(devClass)){
        err("Fail to class create\n");
        return FAIL;
    }

    theDevice = device_create(devClass, NULL, devid, NULL, "nor");
...
{
    int ret = OK;
    int devIndex = spi->modalias[strlen(spi->modalias) - 1];
