...
struct test_result dma_test_result[8];
int	exec_test_number;
...
static int dma_test_main(int id)
{
	...
	case DTC_NORMAL_CONT_MODE:
		ret = __dtc_normal_conti(id);
		break;
	case DTC_DEDICATE:
		ret = __dtc_dedicate(id);
		break;
	case DTC_DEDICATE_CONT_MODE:
		ret = __dtc_dedicate_conti(id);
		break;
	case DTC_ENQ_AFT_DONE:
		ret = __dtc_enq_aftdone(id);
		break;
	case DTC_MANY_ENQ:
		ret = __dtc_many_enq(id);
		break;
	case DTC_CMD_STOP:
		ret = __dtc_stop(id);
		break;
	case DTC_M2M_TWO_THREAD:
		ret = __dtc_two_thread(id);
		break;
	default:
		ret = __LINE__;
		printk("[%s] don`t support this case id: %d! %d\n",__func__,id,__LINE__);
		break;
	}

	if(0 == ret){
		printk("%s: test success!\n", __func__);
		dma_test_result[id].result |= 1;
	}else{
		printk("%s: test failed!\n", __func__);
		dma_test_result[id].result |= 2;
	}
	return ret;
}
			...
			const char *buf, size_t size)
{
	...
	if(0 != dma_test_main(id)){
		pr_err("%s: dma_test_main failed! id %d exec %d\n", __func__, id,exec_test_number);
	}else{
		pr_info("%s: dma_test_main success! id %d exec %d\n", __func__, id,exec_test_number);
	}
	return size;
}
...
ssize_t result_show(struct class *class, struct class_attribute *attr, char *buf)
{
	int i;
	ssize_t cnt = 0;
	for(i=0;i<DTC_MAX;i++){
		cnt += sprintf(buf+cnt,"[%s] result %d! %d\n",dma_test_result[i].name,dma_test_result[i].result,__LINE__);
	}
	return cnt;
}
...
{
	return sprintf(buf,"%u",dma_test_result[exec_test_number].result);
}
ssize_t result_store(struct class *class, struct class_attribute *attr,
			const char *buf, size_t size)
{
	int id = 0;
...
static struct class_attribute dma_test_class_attrs[] = {
	__ATTR(test, 0640, test_show, test_store),
	__ATTR(test, 0640, result_show, result_store),
...
static int __init sw_dma_test_init(void)
{
	...
	for(status=0;status<DTC_MAX;status++){
		dma_test_result[status].name = kmalloc(sizeof(*case_name[status]),GFP_KERNEL);
		memcpy(dma_test_result[status].name,case_name[status],sizeof(*case_name[status]));
		dma_test_result[status].result = 1;
		printk("%s %d\n",dma_test_result[status].name,status);
	}
...
MODULE_ALIAS("platform:sun7i Dma Test");
