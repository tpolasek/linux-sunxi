...
u32 _thread2_proc(int id)
{
	...
	if(0 != uret){
		...
		if(dma_test_result[id].result == 1){
			dma_test_result[id].result = 0;
			pr_info("%s, success!\n", __func__);
			}else{
				pr_info("%s, success! this time but error Before\n", __func__);
			}
	}
...
int __test_thread1(void * arg)
{
	...
	if(0 != uret){
		pr_err("%s err, line %d!\n", __func__, uret); /* print err line */
			dma_test_result[*local_id].result |= 2;
	}else{
		if(dma_test_result[*local_id].result == 1){
			dma_test_result[*local_id].result = 0;
			pr_info("%s, success!\n", __func__);
			}else{
				pr_info("%s, success! this time but error Before\n", __func__);
			}
...
{
	int flag1,flag2,ret = 0,local_id = id;
	/* dump the initial memory status, for test memory leak */
//	__dump_cur_mem_info();
	
	flag1 = kernel_thread(__test_thread1, &local_id, CLONE_FS | CLONE_SIGHAND);
	flag2 = kernel_thread(__test_thread2, &local_id, CLONE_FS | CLONE_SIGHAND);
	if((flag1 <= 0) || (flag2 <= 0)){
		dma_test_result[id].result |= 2;
		printk("flag1 %d flag2 %d\n",flag1,flag2);
		ret  = dma_test_result[id].result;
	}
	return ret;
}
