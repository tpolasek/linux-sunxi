...
static dma_config_t dma_tia[DMA_TEST_CHANNEL];
...
static 	unsigned multichannel_to_one_destination_hd(dma_hdl_t dma_hdl, void *parg)
{
...
static 	unsigned multichannel_to_one_destination_fd(dma_hdl_t dma_hdl, void *parg)
{
	unsigned char *tmp_flag;
	int ret = 1;
	tmp_flag	= parg;
...
static 	unsigned chain_all_channel_test_hd(dma_hdl_t dma_hdl, void *parg)
{
	unsigned char *tmp_flag;
	int ret = 1;
	tmp_flag	= parg;
...
static 	unsigned chain_all_channel_test_fd(dma_hdl_t dma_hdl, void *parg)
{
	unsigned char *tmp_flag;
	int ret = 1;
	tmp_flag	= parg;
...
static 	unsigned enqueue_cb_test_hd(dma_hdl_t dma_hdl, void *parg)
{
	int ret =1;
	u32	psrcu=0,pdstu=0;
	...
	if(enqueue_cb_test_flag & 0x01){
		if(sw_dma_enqueue(dma_hdl, psrcu+1024, pdstu+1024, 1024)){
...
static 	unsigned enqueue_cb_test_fd(dma_hdl_t dma_hdl, void *parg)
{
	int ret =1;
	u32	psrcu=0,pdstu=0;
	struct test_result *struct_result;
	...
	if(0x2 & enqueue_cb_test_flag){
		if(sw_dma_enqueue(dma_hdl, psrcu+1024*2, pdstu+1024*2, 1024)){
...
static 	unsigned enqueue_cb_test_qd(dma_hdl_t dma_hdl, void *parg)
{
	int ret =1;
	u32	psrcu=0,pdstu=0;
	struct test_result *struct_result;
	...
	if(0x4&enqueue_cb_test_flag){
		if(sw_dma_enqueue(dma_hdl, psrcu+1024*3, pdstu+1024*3, 1024)){
			printk("sw_dma_enqueue fail in %s %d\n",__FUNCTION__,__LINE__);
			ret =-1;
		}else{
			struct_result->end_flag++;
...
static	unsigned int single_mode_test_fd_cb(dma_hdl_t dma_hdl, void *parg)
{
	int ret = 1;
	int *d_data,*s_data;
...
static	unsigned int single_continue_test_fd_cb(dma_hdl_t dma_hdl, void *parg)
{
	int ret = 1;
	int *d_data,*s_data;
	struct test_result *struct_result;
	...
	DMATEST_DEBUG("before end_flag %d loop_times %d %d\n",struct_result->end_flag,struct_result->loop_times,__LINE__);
	*s_data	= ++struct_result->end_flag;
	DMATEST_DEBUG("after d_data 0x%x\ns_data 0x%x \n %d\n",readl(d_data),readl(s_data),__LINE__);	

	if(struct_result->end_flag > struct_result->loop_times){
		if(sw_dma_ctl(struct_result->dma_hdl, DMA_OP_STOP, NULL)) {
	...
	DMATEST_DEBUG("after end_flag %d loop_times %d %d\n",struct_result->end_flag,struct_result->loop_times,__LINE__);
	__cpuc_flush_dcache_area(struct_result->vsrcp, 1024 * 1024);
	__cpuc_flush_dcache_area(struct_result->vdstp, 1024 * 1024);
	DMATEST_DEBUG("dma_qd_fd_hd_flag 0x%x %d\n",struct_result->dma_qd_fd_hd_flag,__LINE__);
	struct_result->dma_qd_fd_hd_flag &= (~0x02);
...
static	unsigned int simple_test_hd_cb(dma_hdl_t dma_hdl, void *parg)
{
	int ret = 1;
	struct test_result *struct_result;
	struct_result=parg;
...
static	unsigned int simple_test_fd_cb(dma_hdl_t dma_hdl, void *parg)
{
	int ret = 1;
	struct test_result *struct_result;
	struct_result=parg;
...
static unsigned int enqueue_after_done_test_fd(dma_hdl_t dma_hdl, void *parg)
{
	int ret = 1;
	struct test_result *struct_result;
	struct_result=parg;
	...
	if(dma_test_result[5].end_flag==3)
		struct_result->result |= (1 << 2);	
	ret = 0;
...
*/
	...
	if(sw_dma_enqueue(dma_hdl, psrcu+4, pdstu+4, 4)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}

	if(sw_dma_enqueue(dma_hdl, psrcu+8, pdstu+8, 4)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}
...
static int single_continue_test(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	dma_test_result[2].dma_hdl =dma_hdl;
	vsrcp	= kmalloc(SIZE_1M, GFP_KERNEL);
	psrcu	= virt_to_phys(vsrcp);
	//vsrcp = dma_alloc_coherent(NULL, SIZE_1M, (dma_addr_t *)&psrcu, GFP_KERNEL);
	if(!vsrcp || !psrcu){
...
*/
	while(i && (dma_test_result[2].end_flag < dma_test_result[2].loop_times)){
		i--;
		mdelay(1);
	}
...
static int chain_enqueques_test(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	if(sw_dma_enqueue(dma_hdl, psrcu+1028, pdstu+1028, 4)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}

	if(sw_dma_enqueue(dma_hdl, psrcu+1030, pdstu+1030, 1)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}
...
static int enqueue_after_done_test(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	sw_dma_enqueue(dma_hdl, psrcu+1024, pdstu+1024, 1024);
	
	sw_dma_enqueue(dma_hdl, psrcu+1024*2, pdstu+1024*2, 1024);
	
	sw_dma_enqueue(dma_hdl, psrcu+1024*3, pdstu+1024*3, 1024);
	...
	while(dma_test_result[5].end_flag<3){
		if(i==0){
			if(dma_test_result[5].result & (1 << 2)){
				if(sw_dma_ctl(dma_hdl, DMA_OP_STOP, NULL)) {
					printk("sw_dma_ctl DMA_OP_STOP err! %d\n",__LINE__);
					ret = -1;
					goto ending1;
				}
				dma_test_result[5].result = 0;
				ret = 0;
				goto ending_success;
			}else{
				ret = -1;
...
static int chain_all_channel_test(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	unsigned char tmp_flag;
	dma_hdl_t	dma_hdl[DMA_TEST_CHANNEL];
	void	*vsrcp=NULL,*vdstp=NULL;
	u32		psrcu=0,pdstu=0;
	struct dma_cb_t done_cb;
	struct dma_op_cb_t op_cb;
	...
	for(i=0;i<DMA_TEST_CHANNEL;i++){
		...
		if(!dma_hdl[i]){
			...
			printk("dma_hdl[%d] sw_dma_request success! %d\n",i,__LINE__);
...
static int multichannel_to_one_destination(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	dma_hdl_t	dma_hdl[multichannel_channel_number];
	void	*vsrcp=NULL,*vdstp=NULL;
	u32		psrcu=0,pdstu=0;
	struct dma_cb_t done_cb;
	struct dma_op_cb_t op_cb;
...
static int buffer_not_align(struct sunxi_dma_test_class *sunxi_dma_test)
{
	...
	if(sw_dma_enqueue(dma_hdl, psrcu+2048+3, pdstu+2048+2, 1024)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}

	if(sw_dma_enqueue(dma_hdl, psrcu+4096+2, pdstu+4096+1, 1024)){
		ret = -1;
		printk("sw_dma_enqueue err %d\n",__LINE__);
		goto ending1;
	}
