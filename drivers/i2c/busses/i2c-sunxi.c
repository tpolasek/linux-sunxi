...
#ifdef SUNXI_I2C_FREQ_DEBUG

static ssize_t busfreq_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	...
	clk_in = clk_get_rate(i2c->clk);
	reg_val = readl(i2c->base_addr + TWI_CLK_REG);
	clk_n = TWI_CLK_DIV_N & reg_val;
	clk_m = (TWI_CLK_DIV_M & reg_val) >> 3;
	busfreq = (clk_in >> clk_n) / (10 * (clk_m + 1));

	return sprintf(buf, "%u\n", busfreq);
}

static ssize_t busfreq_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
    ...
    printk("i2c_feq: count = %d\n", count);
	sscanf(buf, "%u", &wanted);
	aw_twi_set_clock(clk_get_rate(i2c->clk), wanted, i2c->base_addr);

	return count;
}

static struct device_attribute bus_freq_attrs[] = {
	__ATTR(busfreq, 0600, busfreq_show, busfreq_store),
};
#endif
...
#ifdef SUNXI_I2C_FREQ_DEBUG
	for (i = 0; i < ARRAY_SIZE(bus_freq_attrs); i++) {
		ret = device_create_file(&dev->dev, &bus_freq_attrs[i]);
		if (ret)
			goto eadapt;
	}
#endif
